class Mario
    ...
    def move()
        ...
        if @mario.x < 0 then
            ...
            $distance_coin += @mario.x - ($width / 2) + $default_tile_size
            $total_distance += @mario.x - ($width / 2) + $default_tile_size
        ...
        @hitbox.y = @mario.y
        $colliders.collision_coin(@hitbox)
    end

    def gravity
        if @dead then
            @velocity_y += 0.4
        elsif $colliders.collision_ground(@hitbox) == 1 or $colliders.collision_ground(@hitbox) == 3 then
            @velocity_y = 0
            @flying = false
        elsif $colliders.collision_hitblock(@hitbox) == 1 then
            @velocity_y = 0.4
        else
            @velocity_y += 0.4
